...
class QuestionController extends Controller
{
    ...
    public function index(Request $request): AnonymousResourceCollection
    {
        ...
        // Randomly select up to 10 high-quality questions from the database
        $dbQuestions = Question::where('level', $level)->inRandomOrder()->take(10)->get();
        ...
        // Generate the remaining questions dynamically with a random seed for infinite variety
        while (count($questions) < 20) {
            $seed = rand(1000, 99999);
            $generated = $this->generateDynamicQuestion($seed, $level, count($questions) + 1);
            
            // Avoid duplicate IDs if randomly generated
            $isDuplicate = false;
    ...
    private function generateDynamicQuestion(int $seed, int $level, int $qIndex)
    {
        // Deterministic pseudo-random number generator based on seed
        $randVal = ($seed * 1103515245 + 12345) & 0x7fffffff;
        
        $type = $randVal % 6; // 6 types of questions
        ...
        if ($type == 0) {
            ...
            $vIdx = ($randVal >> 2) % count($vehicles);
            $v = $vehicles[$vIdx];
            
            $roads = [
                ['name_uz' => 'aholi punktlarida', 'name_en' => 'in populated areas', 'name_ru' => 'в населенных пунктах', 'name_qr' => 'aholı punktlerinde', 'name_cyr' => 'аҳоли пунктларида', 'speed' => 60],
                ...
                ['name_uz' => 'aholi punktlaridan tashqarida', 'name_en' => 'outside populated areas', 'name_ru' => 'вне населенных пунктов', 'name_qr' => 'aholı punktlerinen tısqarıda', 'name_cyr' => 'аҳоли пунктлариdan ташқарида', 'speed' => 90]
            ];
            
            $rIdx = ($randVal >> 4) % count($roads);
            $r = $roads[$rIdx];
            
            $speedLimit = $r['speed'];
            // adjust speed based on vehicle
            if ($vIdx > 0 && $speedLimit == 100) {
                $speedLimit = 90; // buses, trucks and motorcycles on highways is 90
            }
            if ($vIdx == 1 && $speedLimit == 60) {
                $speedLimit = 60; 
            }
            
            $q_uz = "Ushbu holatda: {$r['name_uz']} {$v['name_uz']}ning maksimal ruxsat etilgan tezligi soatiga necha km?";
            $q_cyr = "Ушбу ҳолатда: {$r['name_cyr']} {$v['name_cyr']}нинг максимал рухсат этилган тезлиги соатига неча км?";
            $q_en = "In this situation: what is the maximum speed limit for {$v['name_en']} {$r['name_en']}?";
            $q_ru = "В данной ситуации: какая максимальная скорость разрешена {$v['name_ru']} {$r['name_ru']}?";
            $q_qr = "Usı jaǵdayda: {$r['name_qr']} {$v['name_qr']}niń maksimal ruxsat etilgen tezligi saatına neshe km?";
            
            $opt_correct = $speedLimit;
            $opt_wrong1 = $speedLimit - 20;
            $opt_wrong2 = $speedLimit + 10;
            ...
            $cIdx = ($randVal >> 3) % count($categories);
            $c = $categories[$cIdx];
            
            $q_uz = "Fuqarolar necha yoshdan boshlab {$c['desc_uz']} boshqarish huquqiga ega bo'ladilar?";
            $q_cyr = "Фуқаролар неча ёшдан бошлаб {$c['desc_cyr']} бошқариш ҳуқуқига эга бўладилар?";
            $q_en = "From what age are citizens allowed to drive {$c['desc_en']}?";
            $q_ru = "С какого возраста гражданам разрешается управлять {$c['desc_ru']}?";
            $q_qr = "Puqaralar neshe jasınan baslap {$c['desc_qr']} basqarıw huqıqına iye boladı?";
            
            $age_correct = $c['age'];
            $age_wrong1 = $age_correct - 2;
            $age_wrong2 = $age_correct + 2;
            ...
            $sIdx = ($randVal >> 4) % count($scenarios);
            $s = $scenarios[$sIdx];
            
            $q_uz = "Transport vositasini to'xtatish {$s['desc_uz']} ruxsat etiladi?";
            $q_cyr = "Транспорт воситасини тўхтатиш {$s['desc_cyr']} рухсат этилади?";
            $q_en = "Is it allowed to stop a vehicle {$s['desc_en']}?";
            $q_ru = "Разрешается ли останавливать транспортное средство {$s['desc_ru']}?";
            $q_qr = "Transport quralın toqtatıw {$s['desc_qr']} ruxsat etiledi?";
            
            $d_correct = $s['dist'];
            $d_wrong1 = $d_correct + 5;
            ...
            $vIdx = ($randVal >> 3) % count($violations);
            $v = $violations[$vIdx];
            
            $q_uz = "Haydovchi tomonidan {$v['desc_uz']} holati uchun qancha jarima solinadi?";
            $q_cyr = "Ҳайдовчи томонидан {$v['desc_cyr']} ҳолати учун қанча жарима солинади?";
            $q_en = "What is the penalty for {$v['desc_en']} by the driver?";
            $q_ru = "Какой штраф налагается за {$v['desc_ru']} водителем?";
            $q_qr = "Haydawshı tárepinen {$v['desc_qr']} jaǵdayı ushın qansha jarıma salınadı?";
            
            $f_correct = $v['fine'];
            $f_wrong1 = $f_correct + 1;
            $f_wrong2 = $f_correct + 3;
            ...
            $translations = [
                ...
                'uz_cyr' => [
                    'question' => $q_cyr,
                    'options' => [
                        ['id' => 'a', 'text' => "БҲМнинг {$f_correct} баравари миқдорида"],
                        ['id' => 'b', 'text' => "БҲМнинг {$f_wrong1} баравари миқдорида"],
                        ['id' => 'c', 'text' => "БҲМнинг {$f_wrong2} баравари миқдорида"]
                    ]
                ],
                'en' => [
                    'question' => $q_en,
            ...
            $scenarios = [
                ['desc_uz' => "Chorrahada teng ahamiyatli bo'lmagan yo'llar kesishganda, kim birinchi o'tadi?", 'desc_en' => 'When crossing non-equivalent roads at an intersection, who goes first?', 'desc_ru' => 'При пересечении неравнозначных дорог на перекрестке кто имеет преимущество?', 'desc_qr' => 'Kesiliskede teń áhmiyetli bolmaǵan jollar kesilskende, kim birinshi ótedi?', 'desc_cyr' => "Чорраҳада тенг аҳамиятли бўлмаган йўллар кесишганда, ким биринчи ўтади?"],
            ...
            if ($sIdx == 0) {
                $translations = [
                    ...
                    'uz_cyr' => [
                        'question' => $s['desc_cyr'],
                        'options' => [
                            ['id' => 'a', 'text' => "Бош йўлдан келаётган транспорт воситаси"],
            ...
            // General technical rules (Tire tread, etc.)
            $techs = [
            ...
            $tIdx = ($randVal >> 4) % count($techs);
            $t = $techs[$tIdx];
            
            $q_uz = "Texnik qoidalarga ko'ra, {$t['desc_uz']}";
            $q_cyr = "Техник қоидаларга кўра, {$t['desc_cyr']}";
            $q_en = "According to technical regulations, {$t['desc_en']}";
            $q_ru = "Согласно техническим регламентам, {$t['desc_ru']}";
            $q_qr = "Texnik qaǵıydalarǵa kóre, {$t['desc_qr']}";
            
            $t_correct = $t['limit'];
            $t_wrong1 = ($tIdx == 0) ? '1.0 mm' : '0.5 mm';
            $t_wrong2 = ($tIdx == 0) ? '2.0 mm' : '1.2 mm';
    ...
    public function store(Request $request): QuestionResource
    {
        $validated = $request->validate([
            ...
            'translations.uz_lat' => 'required|array',
            'translations.uz_lat.question' => 'required|string',
            'translations.uz_lat.options' => 'required|array',
            'translations.uz_cyr' => 'required|array',
            'translations.uz_cyr.question' => 'required|string',
            'translations.uz_cyr.options' => 'required|array',
            'translations.ru' => 'required|array',
            'translations.ru.question' => 'required|string',
            'translations.ru.options' => 'required|array',
            'translations.qr' => 'required|array',
            'translations.qr.question' => 'required|string',
            'translations.qr.options' => 'required|array',
    ...
    public function update(Request $request, Question $question): QuestionResource
    {
        $validated = $request->validate([
            ...
            'translations.uz_lat' => 'sometimes|required|array',
            'translations.uz_lat.question' => 'sometimes|required|string',
            'translations.uz_lat.options' => 'sometimes|required|array',
            'translations.uz_cyr' => 'sometimes|required|array',
            'translations.uz_cyr.question' => 'sometimes|required|string',
            'translations.uz_cyr.options' => 'sometimes|required|array',
            'translations.ru' => 'sometimes|required|array',
            'translations.ru.question' => 'sometimes|required|string',
            'translations.ru.options' => 'sometimes|required|array',
            'translations.qr' => 'sometimes|required|array',
            'translations.qr.question' => 'sometimes|required|string',
            'translations.qr.options' => 'sometimes|required|array',
            'correct_option_id' => 'sometimes|required|string',
        ]);
